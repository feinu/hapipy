...
class ContactsClient(BaseClient):
    """
    The hapipy Contacts client uses the _make_request method to call the
    ...
    def _get_path(self, subpath):
        return 'contacts/v%s/%s' % (self.options.get('version') or
                                    CONTACTS_API_VERSION, subpath)
    ...
    def get_contact_by_id(self, contact_id, **options):
        ...
        '''
        return self._call('contact/vid/%s/profile' % contact_id,
                          method='GET', **options)
